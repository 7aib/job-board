...
import type { JobUpdateRequest, JobResponse } from "@/types/job";

interface JobEditFormProps {
  job: JobResponse;
  onSubmit: (jobData: JobUpdateRequest) => Promise<void>;
  loading: boolean;
  serverError?: string | null;
}

export default function JobEditForm({ job, onSubmit, loading, serverError }: JobEditFormProps) {
  const [formData, setFormData] = useState<JobUpdateRequest>({
    title: job.title,
    department: job.department,
    description: job.description,
    location: job.location as JobUpdateRequest["location"],
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (formData.salary_min !== undefined && formData.salary_max !== undefined && formData.salary_min >= formData.salary_max) {
  ...
  const handleChange = (field: keyof JobUpdateRequest, value: string | number | boolean | null | undefined) => {
    setClientError(null);
  ...
  const addSkill = () => {
    ...
    if (skill && formData.required_skills && !formData.required_skills.includes(skill)) {
      setFormData((prev) => ({
        ...prev,
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid gap-6 sm:grid-cols-3">
        <div className="space-y-2">
          ...
          <select
            ...
            onChange={(e) => handleChange("location", e.target.value as JobUpdateRequest["location"])}
            className="w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm text-zinc-900 outline-none transition focus:border-slate-900 focus:ring-2 focus:ring-slate-200 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-100 dark:focus:border-white dark:focus:ring-white/10"
          >
      ...
      <div className="flex items-center gap-3">
        <input
          ...
          className="h-4 w-4 rounded border-zinc-300 text-slate-900 focus:ring-slate-200 dark:border-zinc-600 dark:bg-zinc-950 dark:text-white dark:focus:ring-white/10"
        />
        <label htmlFor="is_closed" className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">
